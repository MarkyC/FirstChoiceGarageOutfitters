<div class="container gallery">

    <h1>Geneva Cabinets</h1>

    <div class="row">
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/locker">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/Locker-open.jpg" alt="Lockers">
                <h1>Lockers</h1>
            </a>
        </div>
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/base">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/base-drawer.jpg" alt="Base Cabinets">
                <h1>Base Cabinets</h1>
            </a>
        </div>
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/service">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/cabinet-service.jpg" alt="Service Cabinets">
                <h1>Service Cabinets</h1>
            </a>
        </div>
    </div>

    <div class="row">
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/wall">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/wall-3.jpg" alt="Wall Storage">
                <h1>Wall Storage</h1>
            </a>
        </div>
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/shelf">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/shelf-3.jpg" alt="Wall-Units">
                <h1>Wall Units</h1>
            </a>
        </div>
        <div class="col-md-4">
            <a href="<?php echo base_url(); ?>cabinets/geneva/bench">
                <img class="img-responsive" src="<?php echo base_url(); ?>img/cabinets/geneva/bench-6.jpg" alt="Benches">
                <h1>Benches</h1>
            </a>
        </div>
    </div>

    <h1>Packages</h1>

    <div class="row">
        <div class="col-md-6 package">
            <img src="<?php echo base_url(); ?>img/cabinets/geneva/package.jpg" alt="Package">
            <h1>Complete Geneva Package</h1>
            <ul>
                <li>An Item</li>
                <li>Another Item</li>
                <li>Another Item</li>
                <li>An Item</li>
                <li>Another Item</li>
                <li>An Item</li>
            </ul>
        </div>
    </div>

</div> <!-- /container -->